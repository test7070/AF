        ...
        <script type="text/javascript">
				...
				for (var i = 1; i < t_set.data('tip').length; i++) {
					...
					if (obj.is(":visible")) {
						t_set.find('.tip').eq(index).show().offset({
							top : round(obj.offset().top + shiftY, 0), left : round(obj.offset().left + obj.width() + shiftX, 0)
						}).html(msg);
					} else {
			...
			function tip(obj, msg, x, y) {
				...
				if ($('#tipClose').length == 0) {
					...
					$('#tipClose').offset({
						top : round($('#btnTip').offset().top - 2, 0), left : round($('#btnTip').offset().left - 15, 0)
					});
					t_set.data('tip').push({
						index : 0, ref : $('#tipClose')
					});
				}
				if (obj.data('tip') == undefined) {
					...
					t_set.data('tip').push({
						index : t_index, ref : obj, msg : msg, shiftX : x, shiftY : y
					});
				}
			}
        ...
        <div class='dbbs'>
            <table id="tbbs" class='tbbs' style=' text-align:center'>
                ...
                <tr  style='background:#cad3ff;'>
                    ...
                    <td>
                    <input id="txtRadius.*" type="text"  class="txt num" style="width:95%;"/>
                    </td>
                    <td>
